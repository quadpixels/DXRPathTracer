...
{
    for(uint32 i = 0; i < ArraySize_(WavefrontThreadGroupSizeVariants); ++i)
    {
        if(uint32(g_wavefront_thread_group_size) == WavefrontThreadGroupSizeVariants[i])
            return i;
    }
...
{
    return WavefrontThreadGroupSizeVariants[WavefrontThreadGroupVariantIndex()];
}
...
struct RayTraceConstants
{
    ...
    uint32 myFlags = 0;
    uint32 WavefrontReadQueue = 0;
    uint32 WavefrontWriteQueue = 1;
...
struct WavefrontShadowWorkItem
{
    Float3 Origin;
    float TMin = 0.0f;
    Float3 Direction;
...
void DXRPathTracer::Shutdown()
{
    ...
    wavefrontPathStateBuffer.Shutdown();
    wavefrontRayQueueA.Shutdown();
    wavefrontRayQueueB.Shutdown();
    wavefrontShadowQueue.Shutdown();
    wavefrontCounterBuffer.Shutdown();
    wavefrontHitQueueA.Shutdown();
    wavefrontHitQueueB.Shutdown();
    wavefrontDispatchArgsBuffer.Shutdown();
    DX12::Release(wavefrontDispatchCommandSignature);
}
...
void DXRPathTracer::DestroyPSOs()
{
    ...
    DX12::DeferredRelease(wavefrontGeneratePrimaryPSO);
    DX12::DeferredRelease(wavefrontTraceHitsPSO);
    ...
    DX12::DeferredRelease(wavefrontPersistentTraceShadowsPSO);
    DX12::DeferredRelease(wavefrontPersistentWorkQueuePSO);
    DX12::DeferredRelease(persistentWarpsPathTracePSO);
    ...
    DX12::DeferredRelease(wavefrontScatterReorderedRaysPSO);
    DX12::DeferredRelease(wavefrontAdvancePSO);
    DX12::DeferredRelease(wavefrontAccumulatePSO);

    for(uint32 variantIdx = 0; variantIdx < ArraySize_(WavefrontThreadGroupSizeVariants); ++variantIdx)
    {
        DX12::DeferredRelease(wavefrontTraceHitsPSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontShadeHitsPSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontTraceShadowsPSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontPersistentTraceShadePSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontPersistentTraceShadowsPSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontPersistentWorkQueuePSOVariants[variantIdx]);
        DX12::DeferredRelease(persistentWarpsPathTracePSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontCountReorderBinsPSOVariants[variantIdx]);
        DX12::DeferredRelease(wavefrontScatterReorderedRaysPSOVariants[variantIdx]);
    }
}
...
void DXRPathTracer::CreateRenderTargets()
{
    ...
    {
        const uint64 numPixels = uint64(width) * uint64(height);

        StructuredBufferInit sbInit;
        sbInit.NumElements = numPixels;
        sbInit.CreateUAV = true;
        sbInit.InitialState = D3D12_RESOURCE_STATE_COMMON;

        sbInit.Stride = sizeof(WavefrontPathState);
        sbInit.Name = L"Wavefront Path States";
        wavefrontPathStateBuffer.Initialize(sbInit);

        sbInit.Stride = sizeof(WavefrontRayWorkItem);
        sbInit.Name = L"Wavefront Ray Queue A";
        const uint64 persistentQueueCapacity = numPixels * Max<uint64>(uint64(AppSettings::MaxPathLength), 1);
        sbInit.NumElements = persistentQueueCapacity;
        wavefrontRayQueueA.Initialize(sbInit);

        sbInit.Name = L"Wavefront Ray Queue B";
        wavefrontRayQueueB.Initialize(sbInit);

        sbInit.NumElements = numPixels;
        sbInit.Stride = sizeof(WavefrontShadowWorkItem);
        sbInit.Name = L"Wavefront Shadow Queue";
        wavefrontShadowQueue.Initialize(sbInit);

        sbInit.Stride = sizeof(WavefrontHitWorkItem);
        sbInit.Name = L"Wavefront Hit Queue A";
        wavefrontHitQueueA.Initialize(sbInit);

        sbInit.Name = L"Wavefront Hit Queue B";
        ...
        RawBufferInit rbInit;
        rbInit.NumElements = 18;
        rbInit.CreateUAV = true;
        rbInit.InitialState = D3D12_RESOURCE_STATE_COMMON;
        rbInit.Name = L"Wavefront Dispatch Args";
        wavefrontDispatchArgsBuffer.Initialize(rbInit);
    }
...
void DXRPathTracer::InitRayTracing()
{
...

    {
        ...
        commandSignatureDesc.pArgumentDescs = &dispatchArg;

        DXCall(DX12::Device->CreateCommandSignature(&commandSignatureDesc, nullptr, IID_PPV_ARGS(&wavefrontDispatchCommandSignature)));
    }
    ...
    wavefrontPersistentTraceShadowsCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontPersistentTraceShadowsCS", ShaderType::Compute, co);
    wavefrontPersistentWorkQueueCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontPersistentWorkQueueCS", ShaderType::Compute, co);
    persistentWarpsPathTraceCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "PersistentWarpsPathTraceCS", ShaderType::Compute, co);
    ...
    wavefrontScatterReorderedRaysCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontScatterReorderedRaysCS", ShaderType::Compute, co);
    wavefrontAdvanceCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontAdvanceCS", ShaderType::Compute, co);
    wavefrontAccumulateCS = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontAccumulateCS", ShaderType::Compute, co);

    for(uint32 variantIdx = 0; variantIdx < ArraySize_(WavefrontThreadGroupSizeVariants); ++variantIdx)
    {
        CompileOptions variantOptions = co;
        variantOptions.Add("WAVEFRONT_THREAD_GROUP_SIZE", WavefrontThreadGroupSizeVariants[variantIdx]);

        wavefrontTraceHitsCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontTraceHitsCS", ShaderType::Compute, variantOptions);
        wavefrontShadeHitsCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontShadeHitsCS", ShaderType::Compute, variantOptions);
        wavefrontTraceShadowsCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontTraceShadowsCS", ShaderType::Compute, variantOptions);
        wavefrontPersistentTraceShadeCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontPersistentTraceShadeCS", ShaderType::Compute, variantOptions);
        wavefrontPersistentTraceShadowsCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontPersistentTraceShadowsCS", ShaderType::Compute, variantOptions);
        wavefrontPersistentWorkQueueCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontPersistentWorkQueueCS", ShaderType::Compute, variantOptions);
        persistentWarpsPathTraceCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "PersistentWarpsPathTraceCS", ShaderType::Compute, variantOptions);
        wavefrontCountReorderBinsCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontCountReorderBinsCS", ShaderType::Compute, variantOptions);
        wavefrontScatterReorderedRaysCSVariants[variantIdx] = CompileFromFile(L"WavefrontPathTrace.hlsl", "WavefrontScatterReorderedRaysCS", ShaderType::Compute, variantOptions);
    }
}
...
void DXRPathTracer::CreateRayTracingRayQueryPSOs() {
  // RayQuery PSOs
  {
    ...
    cpsd.CS = wavefrontClearCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontClearPSO)));

    cpsd.CS = wavefrontGeneratePrimaryCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontGeneratePrimaryPSO)));

    cpsd.CS = wavefrontTraceHitsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontTraceHitsPSO)));

    cpsd.CS = wavefrontShadeHitsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontShadeHitsPSO)));

    cpsd.CS = wavefrontTraceShadowsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontTraceShadowsPSO)));
    ...
    cpsd.CS = wavefrontPersistentTraceShadowsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPersistentTraceShadowsPSO)));

    cpsd.CS = wavefrontPersistentWorkQueueCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPersistentWorkQueuePSO)));

    cpsd.CS = persistentWarpsPathTraceCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&persistentWarpsPathTracePSO)));
    ...
    cpsd.CS = wavefrontCountReorderBinsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontCountReorderBinsPSO)));

    cpsd.CS = wavefrontPrefixReorderBinsCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPrefixReorderBinsPSO)));

    cpsd.CS = wavefrontScatterReorderedRaysCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontScatterReorderedRaysPSO)));

    cpsd.CS = wavefrontAdvanceCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontAdvancePSO)));

    cpsd.CS = wavefrontAccumulateCS.ByteCode();
    DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontAccumulatePSO)));

    for(uint32 variantIdx = 0; variantIdx < ArraySize_(WavefrontThreadGroupSizeVariants); ++variantIdx)
    {
        cpsd.CS = wavefrontTraceHitsCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontTraceHitsPSOVariants[variantIdx])));

        cpsd.CS = wavefrontShadeHitsCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontShadeHitsPSOVariants[variantIdx])));

        cpsd.CS = wavefrontTraceShadowsCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontTraceShadowsPSOVariants[variantIdx])));

        cpsd.CS = wavefrontPersistentTraceShadeCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPersistentTraceShadePSOVariants[variantIdx])));

        cpsd.CS = wavefrontPersistentTraceShadowsCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPersistentTraceShadowsPSOVariants[variantIdx])));

        cpsd.CS = wavefrontPersistentWorkQueueCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontPersistentWorkQueuePSOVariants[variantIdx])));

        cpsd.CS = persistentWarpsPathTraceCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&persistentWarpsPathTracePSOVariants[variantIdx])));

        cpsd.CS = wavefrontCountReorderBinsCSVariants[variantIdx].ByteCode();
        DXCall(DX12::Device->CreateComputePipelineState(&cpsd, IID_PPV_ARGS(&wavefrontCountReorderBinsPSOVariants[variantIdx])));

        cpsd.CS = wavefrontScatterReorderedRaysCSVariants[variantIdx].ByteCode();
...
void DXRPathTracer::RenderRayTracing()
{
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE defaultUAVs[] =
    {
        rtTarget.UAV,
    ...
    DX12::BindTempDescriptorTable(cmdList, defaultUAVs, ArraySize_(defaultUAVs), RTParams_UAVDescriptor, CmdListMode::Compute);
    ...
    rtConstants.WavefrontThreadGroupSize = ActiveWavefrontThreadGroupSize();
    ...
    if(activeRenderPath == 1 && rtPSO_SER == nullptr)
        ...
        activeRenderPath = 0;
    else if(activeRenderPath == 4 && rtRayQueryPSO == nullptr)
        activeRenderPath = 0;
    else if(activeRenderPath == 5 && rtRayQuery1PSO == nullptr)
        activeRenderPath = 0;
    else if(activeRenderPath == 6 && wavefrontTraceHitsPSOVariants[wavefrontVariantIdx] == nullptr)
        activeRenderPath = 5;
    else if(activeRenderPath == 7 && wavefrontPersistentTraceShadePSOVariants[wavefrontVariantIdx] == nullptr)
        activeRenderPath = 6;
    else if(activeRenderPath == 8 && persistentWarpsPathTracePSOVariants[wavefrontVariantIdx] == nullptr)
        activeRenderPath = wavefrontPersistentTraceShadePSOVariants[wavefrontVariantIdx] != nullptr ? 7 : 6;
    else if(activeRenderPath == 9 && wavefrontTraceHitsPSOVariants[wavefrontVariantIdx] == nullptr)
        activeRenderPath = 5;
